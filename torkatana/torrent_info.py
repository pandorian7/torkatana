...
from .types import FilePath, File, TorrentSlice, FileSlice
...
class TorrentInfo:
    ...
    def filePieceRange(self, file_index: int) -> range:
        ts = self.mapFile(file_index)
        start = ts.first_index
        end = ts.first_index + ts.jumps + 1
        if ts.last_size:
            end += 1
